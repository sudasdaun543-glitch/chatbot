...
export default function CoachPanel({ onBack, onDevPanel }: Props) {
  ...
  const exportCsv = () => {
    ...
    const header = ["Оператор", "Email", "Архетип", "Статус", "Оценка", "Сильные стороны", "Ошибки", "Дата"];
    const rows = sessions.map((s) => [
      s.operator_name ?? "",
      s.operator_email ?? "",
      ARCHETYPE_LABELS_CSV[s.archetype] ?? s.archetype,
  ...
  if (!granted) {
    return (
      <div className="coach-panel login-screen">
        ...
        <p className="coach-link" onClick={onBack}>
          ↩ Назад к входу
        </p>
      </div>
    );
  ...
  return (
    <div className="coach-panel">
      ...
      <div className="coach-actions">
        ...
        <button className="btn btn-send" onClick={onDevPanel}>
          🛠 Панель разработчика
        </button>
      </div>
    </div>
